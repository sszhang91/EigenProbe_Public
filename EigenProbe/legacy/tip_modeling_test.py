...
from tip_modeling import SampleResponse, TipResponse
...
def build_rect_eigpairs(Lx=12,Ly=12,Rx=10,Ry=10,Nx=150,Nqmax=None):
...
def simple_test(q=44, _N_sample_eigenbasis=100, _N_tip_eigenbasis=1):
    ...
    eigpairs = build_rect_eigpairs(Lx=2,Ly=2,Nx=101,Nqmax = _N_sample_eigenbasis)

    Sample = SampleResponse(eigpairs,q)
    Tip = TipResponse(Sample.xs,Sample.ys,q=q,fname="./pickle_library/hyperboloid_load_freq=1000.pickle")

    d = Sample.raster_scan(Tip)
